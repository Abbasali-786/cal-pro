...
# Use your Hugging Face token to login
huggingface_token = "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"
...
# Initialize the Gamma2 model pipeline
pipe = pipeline("text-generation", model="google/gemma-2-27b-it")

# Function to get AI response from Gamma2 model
def get_response(user_input):
    """Get response from Gamma2 AI model."""
    ...
    # Call Gamma2 model to get the AI's response
    model_input = [{"role": "user", "content": user_input}]
    ai_message = pipe(model_input)[0]["generated_text"]

    # Append AI's response to session
